...
export class GeminiService {
  ...
  async evaluateWithBaseline(locationData: any, areaCharacteristics: any, baselineScores: any): Promise<any> {
    try {
      ...
      // Validate AI scores against baseline
  ...
  private createEvaluationPrompt(locationData: any, areaCharacteristics: any, baselineScores?: any): string {
    ...
    // Generate baseline scores section if available
...
`;
  ...
  private parseEvaluationResponse(responseText: string): any {
    try {
      ...
      cleanText = cleanText
        ...
        .replace(/,(\s*[}\]])/g, '$1') // Run twice to catch nested cases
        .replace(/\n/g, ' ') // Replace newlines with spaces
        .replace(/\s+/g, ' ') // Normalize whitespace
        .replace(/"\s*,\s*]/g, '"]') // Fix trailing commas in arrays
        .replace(/}\s*,\s*]/g, '}]') // Fix trailing commas after objects in arrays
        .trim();
  ...
  private getCityTier(city: string): string {
    ...
    if (tier1Cities.some(c => cityLower.includes(c))) return 'tier1';
    if (tier2Cities.some(c => cityLower.includes(c))) return 'tier2';
    return 'tier3';
  }
  ...
  private getRegionFromCity(city: string): string {
    ...
    if (eastCities.some(c => cityLower.includes(c))) return 'east';
    if (westCities.some(c => cityLower.includes(c))) return 'west';
    return 'north'; // Default
  }

  /**
   * Add regional market context
   */
  private addRegionalContext(cityTier: string, region: string, config: any): string {
...
- Popular Sides: ${regionalData.preferredSides?.join(', ') || 'chutney, soup'}`;
  ...
  /**
   * Generate detailed business analysis with specific names and ratings
   */
  private generateDetailedBusinessAnalysis(businesses: any): string {
    let analysis = '';
    
    // Restaurants analysis with specific names
  ...
  /**
   * Generate brand presence analysis with specific locations and details
   */
  private generateBrandPresenceAnalysis(brands: any): string {
    let analysis = '';
    
    const foundBrands = Object.entries(brands).filter(([brand, brandData]: [string, any]) => {
      const places = Array.isArray(brandData) ? brandData : (brandData?.places || []);
      return places.length > 0;
    });
    
    const notFoundBrands = Object.entries(brands).filter(([brand, brandData]: [string, any]) => {
      const places = Array.isArray(brandData) ? brandData : (brandData?.places || []);
      return places.length === 0;
    });
    ...
    if (foundBrands.length > 0) {
      ...
      foundBrands.forEach(([brand, brandData]: [string, any]) => {
        const places = Array.isArray(brandData) ? brandData : (brandData?.places || []);
  ...
  /**
   * Get tier-specific business insights
   */
  private getTierSpecificBusinessInsights(cityTier: string): string {
    switch(cityTier) {
      case 'tier1':
        return `
...
- VIABILITY: VERY GOOD - Strong growth potential with lower risks`;
  ...
  /**
   * Generate contextual scoring guidelines
   */
  private generateContextualScoringGuidelines(locationData: any, cityTier: string): string {
    const { businesses, brands, summary } = locationData;
    const totalBusinesses = summary.overall?.totalBusinesses || 0;
    const brandCount = Object.values(brands).filter((b: any) => 
      Array.isArray(b) ? b.length > 0 : (b?.found || false)
    ).length;
...
${this.getTargetAudienceScoringGuideline(cityTier)}`;
  ...
  /**
   * Get city tier scoring guidelines
   */
  private getCityTierScoringGuideline(cityTier: string): string {
    switch(cityTier) {
      case 'tier1':
...
- Cost advantages for sustainable business`;
  ...
  /**
   * Get food brand scoring guidelines
   */
  private getFoodBrandScoringGuideline(brandCount: number, cityTier: string): string {
    if (cityTier === 'tier1') {
  ...
  /**
   * Get competition scoring guidelines
   */
  private getCompetitionScoringGuideline(competitionLevel: number, cityTier: string): string {
    if (cityTier === 'tier1') {
  ...
  /**
   * Get university proximity guidelines
   */
  private getUniversityProximityGuideline(universities: any[]): string {
    const count = universities.length;
    const avgDistance = count > 0 ? this.calculateAverageDistance(universities) : 1000;
    
    if (count >= 3 && avgDistance <= 500) return `Score 5: Excellent (${count} universities within 500m)`;
    if (count >= 2 && avgDistance <= 800) return `Score 4: Good (${count} universities within 800m)`;
    if (count >= 1 && avgDistance <= 1000) return `Score 3: Average (${count} university within 1km)`;
    return `Score 2: Poor (${count} universities, distant or none)`;
  }

  /**
   * Calculate average distance for places
   */
  private calculateAverageDistance(places: any[]): number {
    if (!places.length) return 0;
    
    // Simple approximation - in real implementation would calculate from center point
    return 600; // Average distance approximation
  }

  /**
   * Get target audience scoring guidelines
   */
  private getTargetAudienceScoringGuideline(cityTier: string): string {
...
- Score 1: No colleges/universities + very low commercial activity`;
  ...
  /**
   * Generate baseline scores section for prompt
   */
  private generateBaselineScoresSection(baselineScores: any): string {
    let section = `
...
`;
    ...
    Object.entries(baselineScores).forEach(([param, scoreData]: [string, any]) => {
...
`;
  ...
  /**
   * Validate AI scores against data-driven baseline (prevent uniform scoring)
   */
  private validateAIScores(aiScores: any, baselineScores: any): any {
    ...
    Object.keys(baselineScores).forEach(param => {
      ...
      // Allow only ±1 adjustment from data baseline
  ...
  /**
   * Get parameter weight from configuration
   */
  private getParameterWeight(parameterId: string): number {
    const weights = {
